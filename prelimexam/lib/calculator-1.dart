...
class _CalculatorScreenState extends State<CalculatorScreen> {
  ...
  void _onOperatorPressed(String operator) {
    ...
    if(_firstOperand.isEmpty) {
      ...
      // If there's already a first operand and operator, calculate the result first
      _calculate();
      setState(() {
        _firstOperand = _display;
        _operator = operator;
    ...
    setState(() {
      ...
      switch (function) {
        ...
        case '√':
          if (num < 0) {
            _display = 'Error';
            _expression = 'Cannot take √ of negative number';
            _resetAfterError();
            return;
          }
          result = sqrt(num);
        ...
        case 'log':
          if (num <= 0) {
            _display = 'Error';
            _expression = 'Cannot take log of non-positive number';
            _resetAfterError();
            return;
          }
          result = log(num) / ln10;
        ...
        case 'ln':
          if (num <= 0) {
            _display = 'Error';
            _expression = 'Cannot take ln of non-positive number';
            _resetAfterError();
            return;
          }
          result = log(num);
